...
const ADMIN_PIN = "@1914"; // altere para sua senha

export default function App() {
  const [view,         setView]        = useState("agenda");
  const [agendamentos, setAgendamentos]= useState([]);
  const [membros,      setMembros]     = useState(DEFAULT_MEMBERS);
  ...
  useEffect(() => {
    ...
    return () => { unsubAuth(); unsubAg(); unsubMb(); };
  }, []);
  ...
  async function addAgendamento(ag) { await addDoc(collection(db, "agendamentos"), ag); }
  async function removeAgendamento(id) { await deleteDoc(doc(db, "agendamentos", id)); }
  ...
  async function handleSubmit() {
    ...
    await addAgendamento({ ...form, uid: user.uid, criadoEm: new Date().toISOString() });
    setFormSuccess(true);
  ...
  async function handleAdicionarMembro() {
    ...
    const nova = [...membros, nome].sort((a,b)=>a.localeCompare(b));
    await saveMembros(nova);
    setNovoNome("");
  }
  ...
  if (!user) return (
    <div style={{...S.root, display:"flex", alignItems:"center", justifyContent:"center"}}>
      <style>{"@keyframes spin { to { transform: rotate(360deg) } }"}</style>
      <div style={{background:"#fff", borderRadius:16, padding:"48px 36px", maxWidth:380, width:"100%", textAlign:"center", boxShadow:"0 4px 24px rgba(0,0,0,0.1)", margin:16}}>
        <div style={{fontSize:48, marginBottom:16}}>🛒</div>
        <h1 style={{fontSize:22, fontWeight:700, color:"#1a2a3a", marginBottom:8}}>Carrinho de Publicacoes</h1>
        <p style={{color:"#7a9ab8", fontSize:14, marginBottom:32}}>Testemunho Publico — Faca login para continuar</p>
        <button onClick={handleGoogleLogin} style={{display:"flex", alignItems:"center", justifyContent:"center", gap:12, width:"100%", padding:"13px 20px", background:"#fff", border:"2px solid #dce6f0", borderRadius:10, cursor:"pointer", fontSize:15, fontWeight:600, color:"#1a2a3a", boxShadow:"0 2px 8px rgba(0,0,0,0.08)", transition:"all 0.2s"}}>
          <svg width="20" height="20" viewBox="0 0 48 48">
            <path fill="#FFC107" d="M43.6 20H24v8h11.3C33.6 32.8 29.3 36 24 36c-6.6 0-12-5.4-12-12s5.4-12 12-12c3 0 5.8 1.1 7.9 3l5.7-5.7C34.1 6.5 29.3 4 24 4 12.9 4 4 12.9 4 24s8.9 20 20 20c11 0 20-8.9 20-20 0-1.3-.1-2.7-.4-4z"/>
            ...
            <path fill="#4CAF50" d="M24 44c5.2 0 9.9-1.9 13.5-5l-6.2-5.2C29.5 35.5 26.9 36 24 36c-5.2 0-9.6-3.2-11.4-7.7l-6.6 5.1C9.5 39.4 16.3 44 24 44z"/>
  ...
  return (
    <div style={S.root}>
      ...
      <header style={S.header} className="mob-header">
        <div style={{...S.hInner, display:"flex", justifyContent:"space-between", alignItems:"center"}}>
          ...
          <div style={{display:"flex", alignItems:"center", gap:10}}>
            <img src={user.photoURL} alt="" style={{width:32,height:32,borderRadius:"50%",border:"2px solid rgba(255,255,255,0.5)"}} />
            <button onClick={handleLogout} style={{background:"rgba(255,255,255,0.2)",border:"1px solid rgba(255,255,255,0.4)",color:"#fff",padding:"6px 12px",borderRadius:6,cursor:"pointer",fontSize:12}}>
              Sair
            </button>
          </div>
        </div>
          ...
          <div>
            ...
            <div style={S.dGrid}>
              {agendaDays.map(dia => {
                ...
                const diaAg = agendamentos.filter(a=>a.data===key).sort((a,b)=>a.horaInicio.localeCompare(b.horaInicio));
                    ...
                    {diaAg.length>0 && (
                      <div style={{padding:"8px 10px",display:"flex",flexDirection:"column",gap:6}}>
                        {diaAg.map(a => {
                          ...
                          const isOwner = a.uid === user.uid;
                          return (
                            <div key={a.id} style={{...S.agCard,...(isOwner?{borderLeft:"3px solid #2a90ff"}:{})}} onClick={()=>setModal(a)}>
        ...
        {view==="meus" && (
          <div>
            <div style={{...S.formCard,maxWidth:500,marginBottom:16}} className="mob-formcard">
              ...
              <p style={{color:"#7a9ab8",fontSize:13,marginTop:4}}>Logado como <strong style={{color:"#1a6abf"}}>{user.displayName}</strong></p>
            </div>
            {(() => {
              const meus = agendamentos
                .filter(a=>a.uid===user.uid)
                .sort((a,b)=>(a.data+a.horaInicio).localeCompare(b.data+b.horaInicio));
              if (!meus.length) return <p style={{color:"#7a9ab8",textAlign:"center",marginTop:24}}>Nenhum agendamento encontrado.</p>;
              return (
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  {meus.map(a => {
                    ...
                    return (
                      <div key={a.id} style={{...S.agCard, borderLeft:"3px solid #2a90ff"}} onClick={()=>setModal(a)}>
      ...
      {/* MODAL */}
      {modal && (() => {
        const ms = MOSTRUARIOS.find(m=>m.id===modal.mostruario);
        const d  = new Date(modal.data+"T12:00:00");
        const isOwner = modal.uid === user.uid;
        return (
