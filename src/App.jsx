...
export default function App() {
  ...
  useEffect(() => {
    ...
    return () => { unsubAuth(); unsubAg(); unsubFx(); unsubMb(); };
  }, []);

  async function handleGoogleLogin() {
    try { await signInWithPopup(auth, provider); } catch(e) { console.error(e); }
  }
  async function handleLogout() { await signOut(auth); setAdminOk(false); }
  ...
  async function handleSubmit() {
    ...
    if (form.fixo) {
      ...
      await addDoc(collection(db, "fixos"), {
        ...
        uid: user.uid, criadoEm: new Date().toISOString(),
      });
    } else {
  ...
  if (!user) return (
    <div style={{...S.root, display:"flex", alignItems:"center", justifyContent:"center"}}>
      ...
      <div style={{background:"#fff", borderRadius:16, padding:"48px 36px", maxWidth:380, width:"100%", textAlign:"center", boxShadow:"0 4px 24px rgba(0,0,0,0.1)", margin:16}}>
        ...
        <button onClick={handleGoogleLogin} style={{display:"flex", alignItems:"center", justifyContent:"center", gap:12, width:"100%", padding:"13px 20px", background:"#fff", border:"2px solid #dce6f0", borderRadius:10, cursor:"pointer", fontSize:15, fontWeight:600, color:"#1a2a3a", boxShadow:"0 2px 8px rgba(0,0,0,0.08)"}}>
      ...
      <main style={S.main} className="mob-main">
        ...
        {view==="meus" && (
          <div>
            ...
            {/* Fixos */}
            {fixos.filter(f=>f.uid===user.uid).length > 0 && (
              <div style={{marginBottom:16}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  {fixos.filter(f=>f.uid===user.uid).map(f => {
            ...
            {/* Normais */}
            {(() => {
              const meus = agendamentos.filter(a=>a.uid===user.uid)
                .sort((a,b)=>(a.data+a.horaInicio).localeCompare(b.data+b.horaInicio));
              if (!meus.length && fixos.filter(f=>f.uid===user.uid).length===0)
