...
const STORAGE_KEY = "carrinho-agendamentos-v1";
...
const MEMBERS_KEY = "carrinho-membros-v1";
...
export default function App() {
  ...
  const PIN = "1234"; // altere para sua senha
  ...
  useEffect(() => {
    ...
    const ag = loadData(STORAGE_KEY);
    const mb = loadData(MEMBERS_KEY);
    if (ag) setAgendamentos(ag);
    if (mb && mb.length) setMembros(mb);
    setLoading(false);
  }, []);

  function persist(newAg) { setAgendamentos(newAg); saveData(STORAGE_KEY, newAg); }
  function persistMembros(list) { setMembros(list); saveData(MEMBERS_KEY, list); }
  ...
  function handleSubmit() {
    ...
    persist([...agendamentos, { ...form, id: Date.now().toString(), criadoEm: new Date().toISOString() }]);
    setFormSuccess(true);
    setTimeout(() => { setFormSuccess(false); setView("agenda"); }, 2000);
  }

  function handleCancelar(id) { persist(agendamentos.filter(a => a.id !== id)); setModal(null); }
  ...
  function handleRemoverMembro(nome) {
    if (!window.confirm(`Remover "${nome}" da lista?`)) return;
    persistMembros(membros.filter(m => m !== nome));
  }

  function handlePin() {
    if (pinInput === PIN) { setAdminOk(true); setPinError(false); setPinInput(""); }
    else { setPinError(true); }
  }

  const agendaDays = Array.from({ length: 7 }, (_,i) => addDays(today(), agendaOffset*7+i));

  if (loading) return <div style={S.root} />;
                        ...
                        {diaAg.map(a => {
                          ...
                          return (
                            <div key={a.id} style={S.agCard} onClick={()=>setModal(a)}>
                              <div style={{fontSize:12,fontWeight:"bold",color:"#4a9eff",minWidth:95}}>
          ...
          <div>
            ...
            {form.nome && (() => {
              const meus = agendamentos
                .filter(a=>a.nome===form.nome||a.dupla===form.nome)
                .sort((a,b)=>(a.data+a.horaInicio).localeCompare(b.data+b.horaInicio));
              if (!meus.length) return <p style={{color:"#3a6080",textAlign:"center",marginTop:24}}>Nenhum agendamento encontrado.</p>;
              return (
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  {meus.map(a => {
                    ...
                    return (
                      <div key={a.id} style={S.agCard} onClick={()=>setModal(a)}>
                        <div style={{fontSize:12,fontWeight:"bold",color:"#4a9eff",minWidth:95}}>{a.horaInicio} - {a.horaFim}</div>
                        {ms?.img && <img src={ms.img} alt={ms.label} style={{width:22,height:30,objectFit:"contain"}} />}
                        <div style={{flex:1,display:"flex",flexDirection:"column",gap:2}}>
            ...
            <div style={S.formCard} className="mob-formcard">
              ...
              {!adminOk ? (
                <div style={{textAlign:"center",padding:"24px 0"}}>
                  <p style={{color:"#7aa8d8",marginBottom:24,fontSize:14}}>
                  ...
                  />
                  {pinError && <p style={{color:"#e07070",fontSize:13,marginTop:12}}>PIN incorreto. Tente novamente.</p>}
                  <div>
                    <button style={{...S.subBtn, marginTop:16, maxWidth:200, display:"inline-block"}} onClick={handlePin}>
                      Entrar
                    </button>
                  </div>
                </div>
                ...
                <div>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24}}>
                    <p style={{color:"#7aa8d8",fontSize:13}}>
                      {membros.length} participante{membros.length!==1?"s":""} cadastrado{membros.length!==1?"s":""}
                    </p>
                  ...
                  <label style={{...S.lbl, marginTop:28}}>
                      ...
                      return (
                        <div key={m} style={{display:"flex",alignItems:"center",gap:12,padding:"11px 14px",background:"rgba(74,158,255,0.04)",border:"1px solid #152540",borderRadius:8}}>
                          ...
                          <div style={{flex:1}}>
                            ...
                            <span style={{color:"#3a6080",fontSize:11,marginLeft:10}}>
                              {agCount} agendamento{agCount!==1?"s":""}
                            </span>
                          </div>
        ...
        return (
          <div style={S.overlay} onClick={()=>setModal(null)}>
            <div style={S.modalCard} onClick={e=>e.stopPropagation()}>
              ...
              <p style={{fontSize:17,margin:"10px 0 4px",color:"#c8d8f0",fontWeight:"bold"}}>
                {modal.horaInicio} - {modal.horaFim}
              </p>
...
const S = {
  ...
  row3:      { display:"flex", gap:12, flexWrap:"wrap", marginTop:4 },
